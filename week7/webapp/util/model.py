...
def get_user_data(user_account, user_password_signin):                       #獲取使用者資訊，用來登入判斷，確認使用者帳號及密碼
        ...
        try:
                sql = "SELECT username, password, name FROM member WHERE username = %s"   #透過輸入帳號，從資料庫獲取 帳號 及 密碼
                val = (user_account,)
                mycursor.execute(sql, val)
                user_data_in_db = mycursor.fetchall()
                if user_data_in_db:                                                        #如果使用者帳號存在於資料庫就進行密碼比對
                        user_password_signup = user_data_in_db[0][1]                       #將使用者密碼取出
                        user_name__signup = user_data_in_db[0][2]                          #將使用者名稱取出   
                        if user_password_signup == user_password_signin:                   #如果使用者密碼與輸入密碼相符 則登入成功    
                                return True,user_name__signup, "Auth sucess"               #回傳登入訊息並轉跳頁面
                        else:
                                return False,False, "You account or passord need to check" #登入失敗就回傳帳號或密碼有錯誤
                else:
                        return False,False, "You don't have signup"                        #如果找不到使用者帳號，回傳尚未註冊
        except mysql.connector.Error as err:
                return False,False, "You don't have signup"
